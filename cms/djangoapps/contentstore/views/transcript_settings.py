...
def transcript_credentials_handler(request, course_key_string):
    ...
    Returns:
        - An ok response if credentials are updated in edx-video-pipeline.
        - A 404 response if transcript feature is not enabled.
        - A 400 if credentials do not pass validations, hence not updated in pipeline.
    """
    ...
    if error_message:
        response = JsonResponse({'success': False, 'message': error_message}, status=400)
    else:
        ...
        # TODO: @mushtaq mine the response in case of an error from pipeline service?
        response = JsonResponse({'success': is_updated, 'message': content}, status=200 if is_updated else 400)
